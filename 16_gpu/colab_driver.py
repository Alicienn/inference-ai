...
"""Pilote autonome : ecrit la source CUDA, compile, execute, affiche tout.
Lance avec :  colab run --gpu T4 colab_driver.py
"""
...
print("=" * 90)
rc, out, err = sh("nvidia-smi --query-gpu=name,memory.total,clocks.max.memory --format=csv")
print(out or err)
rc, cc, _ = sh("nvidia-smi --query-gpu=compute_cap --format=csv,noheader")
cc = (cc.strip().splitlines() or ["7.5"])[0].strip()
arch = "sm_" + cc.replace(".", "")
print("compute capability " + cc + " -> " + arch)
print("=" * 90)

pathlib.Path("asp_bench.cu").write_text(SRC)
rc, out, err = sh("nvcc -O3 -arch=" + arch + " asp_bench.cu -o asp_bench")
if rc != 0:
