# -*- coding: utf-8 -*-
"""FRONTIERE DE BUDGET a Lb=4 : quelle est la plus petite fraction de cles qui
egale encore l'attention dense ? Budgets 128 et 160 cles (25 % et 31,2 % de T=512),
plusieurs repartitions (W, m), et le controle fenetre pure a W=128."""
...
T, DP, NFIT, LB = 512, 8, 256, 4
CONFIGS = [(64, 16), (32, 24), (128, 0), (64, 24), (32, 32), (128, 8)]
OFFSETS = [0, 200000, 400000]
CFG = {"mode": "sparse", "sel": "pca", "W": 128, "Lb": LB, "m": 16}
...
log = OUT / "frontiere_budget.txt"
log.write_text(f"{MID} | T={T} Lb={LB} d'={DP} | index PCA CAUSAL\n"
               "W    m   cles  frac   offset  dense   maxip   pca     vs_dense  index\n",
               encoding="utf-8")
...
for off in OFFSETS:
    ...
    for (W, M) in CONFIGS:
        ...
        acc.setdefault((W, M), []).append((ld, lm, lp))
        with log.open("a", encoding="utf-8") as f:
            f.write(f"{W:4d} {M:4d} {W+M*LB:5d} {(W+M*LB)/T:6.3f} {off:7d} {ld:7.4f} "
                    f"{lm:7.4f} {lp:7.4f} {lm-ld:+9.4f} {lp-lm:+8.4f}\n")
            f.flush()
        print(f"off={off} W={W} m={M} cles={W+M*LB} vs_dense={lm-ld:+.4f} index={lp-lm:+.4f}",
              flush=True)
with log.open("a", encoding="utf-8") as f:
    f.write("\n=== moyennes 3 tranches ===\nW    m   cles  frac   vs_dense  index\n")
    for (W, M), v in acc.items():
        vd = sum(x[1] - x[0] for x in v) / len(v)
        ic = sum(x[2] - x[1] for x in v) / len(v)
        f.write(f"{W:4d} {M:4d} {W+M*LB:5d} {(W+M*LB)/T:6.3f} {vd:+9.4f} {ic:+8.4f}\n")
print("FIN")
