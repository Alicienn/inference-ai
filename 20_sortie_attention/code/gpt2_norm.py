...
def make_fwd(li):
    def fwd(self, hidden_states, layer_past=None, attention_mask=None, head_mask=None,
        ...
        if CFG["mode"] == "pca" and m > 0 and qn == T:
            ...
            del prod, qp, kp
            if CFG.get("bscore", "pca") in ("norm", "normsum"):
                kn = k[0].norm(dim=-1)[:, :nb * LB].view(h, nb, LB)
                Bq = kn.amax(-1) if CFG["bscore"] == "norm" else kn.sum(-1)
                B = Bq[:, None, :].expand(h, qn, nb).contiguous()
                del kn, Bq
            else:
            ...
            if two == "none":
                ...
                elif two in ("q8h", "q4h", "q8t", "q4t"):
                    bits = 8 if two.startswith("q8") else 4
                    lv = float(2 ** (bits - 1) - 1)
                    if two.endswith("h"):
                        scl = k[0].abs().amax(dim=(1, 2), keepdim=True)
                    else:
                        scl = k[0].abs().amax()
                    kq = torch.round(k[0] / scl.clamp_min(1e-8) * lv).clamp(-lv, lv) / lv * scl
                    scq = torch.einsum("hqd,hkd->hqk", q[0], kq)
                    del kq
                    Bc = scq[:, :, :nb * LB].view(h, qn, nb, LB).amax(-1).gather(2, cand)
                elif two == "exactmass":
...
def run(dp, kagg, bagg, m=16, W=32, two="none", bscore="pca"):
    global D
    if dp is None:
        CFG["mode"] = "dense"; CFG["W"] = T // 8; CFG["m"] = 0
    else:
        CFG["mode"] = "pca"; CFG["dp"] = dp; CFG["kagg"] = kagg; CFG["bagg"] = bagg
        CFG["W"] = W; CFG["m"] = m; CFG["two"] = two; CFG["bscore"] = bscore
    ...
    with log.open("a", encoding="utf-8") as f:
        f.write(f"{m:4d}  {CFG['bscore']:8s}  {l4:9.5f}  {l4 - D:+.5f}  {pk:5.1f}  "
                f"{Sm:.4f}  {L1:.4f}\n")
    print(f"m={m} bscore={CFG['bscore']}: perte={l4:.5f} Delta={l4-D:+.5f} pic={pk:.1f}% "
          f"S={Sm:.4f} L1={L1:.4f}", flush=True)
    rows.append((f"m{m}-{CFG['bscore']}", l4 - D, pk, Sm, L1))
    return l4
...
for mm in (8, 16, 24):
    ...
    for bs in ("pca", "norm", "normsum"):
        run(8, "max", "max", m=mm, W=32, two="none", bscore=bs)
