...
log = OUT / "gpt2_metrique.txt"
log.write_text(f"{MID} | banc induction | frac=0,375 | S = masse retenue, L1 = |sparse-dense|\n"
               "dp  kagg  bagg  perte      Delta     pic%   S       L1\n", encoding="utf-8")
rows = []
D = 0.0
...
def run(dp, kagg, bagg):
    ...
    rows.append((f"d{dp}-{kagg}/{bagg}", l4 - D, pk, Sm, L1))
    return l4


D = run(None, None, None)
for cfg in [(8, "max", "max"), (8, "sum", "max"), (8, "sum", "sum"),
            (16, "sum", "max"), (32, "sum", "max"), (32, "max", "max")]:
    run(*cfg)
