# -*- coding: utf-8 -*-
"""L'AGGREGATION est-elle le verrou ? Banc GPT-2, frac=0,375 fixe (W=32, m=16).
kagg : max ou somme sur les projections (score d'une cle).
bagg : max ou somme sur les cles d'un bloc (score d'un bloc)."""
...
log = OUT / "gpt2_agregation.txt"
log.write_text(f"{MID} | banc induction | frac=0,375 (W=32, m=16) | Lb={LB}\n"
               "dp  kagg  bagg  perte_copie4  Delta    pic_retenu%\n", encoding="utf-8")
D = 0.0
...
def run(dp, kagg, bagg):
    ...
    with log.open("a", encoding="utf-8") as f:
        f.write(f"{dp:3d}  {kagg:4s}  {bagg:4s}  {l4:12.5f}  {l4 - D:+.5f}  {pk:6.1f}\n")
    print(f"dp={dp} kagg={kagg} bagg={bagg}: perte={l4:.5f} Delta={l4-D:+.5f} pic={pk:.1f}%",
          flush=True)
    return l4
