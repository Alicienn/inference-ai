    ...
    def fwd(self, hidden_states, layer_past=None, attention_mask=None, head_mask=None,
        ...
        if CFG["mode"] == "pca" and m > 0 and qn == T:
            ...
            if CFG.get("vsel", "none") != "none":
                ...
                if vs in ("v3", "rnd3"):
                    _, s1p = torch.topk(B * elig[None], 1, dim=2)
                    B = B.scatter(2, s1p, 1e9)
                del vd, bv, d
...
log = OUT / "gpt2_vsel3.txt"
log.write_text(f"{MID} | banc induction | score sensible a la valeur | S = masse retenue, L1 = |sparse-dense|\n"
               "m     vsel  perte      Delta     pic%   S       L1\n", encoding="utf-8")
rows = []
D = 0.0


def run(dp, kagg, bagg, m=16, W=32, vsel="none"):
    global D
    if dp is None:
        CFG["mode"] = "dense"; CFG["W"] = T // 8; CFG["m"] = 0
    else:
        CFG["mode"] = "pca"; CFG["dp"] = dp; CFG["kagg"] = kagg; CFG["bagg"] = bagg
        CFG["W"] = W; CFG["m"] = m; CFG["vsel"] = vsel
    ...
    with log.open("a", encoding="utf-8") as f:
        f.write(f"{m:4d}  {CFG['vsel']:5s}  {l4:9.5f}  {l4 - D:+.5f}  {pk:5.1f}  "
                f"{Sm:.4f}  {L1:.4f}\n")
    print(f"m={m} vsel={CFG['vsel']}: perte={l4:.5f} Delta={l4-D:+.5f} pic={pk:.1f}% "
          f"S={Sm:.4f} L1={L1:.4f}", flush=True)
    rows.append((f"m{m}-{CFG['vsel']}", l4 - D, pk, Sm, L1))
    return l4
...
for mm in (8, 16, 24):
    ...
    for vs in ("none", "v3", "rnd3"):
        run(8, "max", "max", m=mm, W=32, vsel=vs)
