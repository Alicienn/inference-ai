...
def make_fwd(li):
    def fwd(self, hidden_states, layer_past=None, attention_mask=None, head_mask=None,
        ...
        if CFG["mode"] == "pca" and m > 0 and qn == T:
            ...
            if CFG.get("vsel", "none") != "none":
                vv = v[0]                                        # (h,qn,hd)
                vs = CFG["vsel"]
                if vs in ("v1", "anti"):
                    p2 = sc.argmax(-1)
                    d = vv[torch.arange(h, device=vv.device)[:, None], p2]
                    if vs == "anti":
                        d = -d
                elif vs == "v2":
                    _, s1 = torch.topk(B * elig[None], 1, dim=2)
                    k1 = (s1[..., None] * LB + torch.arange(LB)).reshape(h, qn, LB)
                    w = A3.gather(2, k1)
                    w = w / w.sum(-1, keepdim=True).clamp_min(1e-9)
                    vk = vv[torch.arange(h, device=vv.device)[:, None, None], k1]
                    d = (vk * w[..., None]).sum(2)
                else:  # rnd : controle, direction aleatoire fixe par tete
                    d = torch.randn(h, 1, vv.shape[-1], device=vv.device).expand(h, qn, vv.shape[-1])
                d = d / d.norm(dim=-1, keepdim=True).clamp_min(1e-9)
                vd = torch.einsum("hqd,hkd->hqk", d, vv)          # (h,qn,qn)
                bv = vd[:, :, :nb * LB].view(h, qn, nb, LB).mean(-1)
                g = 1.0 / bv.std().clamp_min(1e-6)
                B = B * torch.clamp(1.0 + g * bv, min=0.0)
                del vd, bv, d
            selm = None
            if CFG.get("tau") is None:
                ...
                selm = torch.zeros_like(Bm, dtype=torch.bool).scatter_(2, sel, True)
                top1 = Bm.amax(dim=2, keepdim=True)
                keepthr = Bm >= CFG["tau"] * top1
                _, s1 = torch.topk(Bm, 1, dim=2)
                keepthr = keepthr | torch.zeros_like(Bm, dtype=torch.bool).scatter_(2, s1, True)
                selm = selm & keepthr
                ACC["nblk"] += int(selm[:, QPOS].sum())
                ACC["ncell"] += int(selm[:, QPOS].shape[0] * selm[:, QPOS].shape[1])
                mask = torch.zeros(h, qn, qn, dtype=torch.bool)
                mask[:, :, :nb * LB] = selm[..., None].expand(h, qn, nb, LB).reshape(h, qn, nb * LB)
            mask |= (j[None, None, :] > (j[None, :, None] - W))
            pk = A3.argmax(-1)
            pkb = pk // LB
            hit = (sel == pkb[..., None]).any(-1) if selm is None else selm.gather(2, pkb[..., None]).squeeze(-1)
...
log = OUT / "gpt2_tau.txt"
log.write_text(f"{MID} | banc induction | budget adaptatif (seuil tau * meilleur score, plafond m) | S = masse retenue\n"
               "m     tau   nblk   perte      Delta     pic%   S       L1\n", encoding="utf-8")
rows = []
D = 0.0


def run(dp, kagg, bagg, m=16, W=32, vsel="none", tau=None):
    global D
    if dp is None:
        CFG["mode"] = "dense"; CFG["W"] = T // 8; CFG["m"] = 0
    else:
        CFG["mode"] = "pca"; CFG["dp"] = dp; CFG["kagg"] = kagg; CFG["bagg"] = bagg
        CFG["W"] = W; CFG["m"] = m; CFG["vsel"] = vsel; CFG["tau"] = tau
        ACC["nblk"] = 0; ACC["ncell"] = 0
    ...
    with log.open("a", encoding="utf-8") as f:
        fr = ACC["nblk"] / max(ACC["ncell"], 1)
        f.write(f"{m:4d}  {tau:4.1f}  {fr:5.2f}  {l4:9.5f}  {l4 - D:+.5f}  {pk:5.1f}  "
                f"{Sm:.4f}  {L1:.4f}\n")
    fr2 = ACC["nblk"] / max(ACC["ncell"], 1)
    print(f"m={m} tau={tau}: nblk={fr2:.2f} perte={l4:.5f} Delta={l4-D:+.5f} pic={pk:.1f}% "
          f"S={Sm:.4f} L1={L1:.4f}", flush=True)
    rows.append((f"m{m}-tau{tau}", l4 - D, pk, Sm, L1))
    return l4


D = run(None, None, None)
for mm in (8, 16, 24):
    print(f"### plafond m={mm}, W=32, budget fixe = {(32 + mm*4)/256:.3f}", flush=True)
    for ta in (0.0, 0.3, 0.5, 0.7, 0.9, 1.0):
        run(8, "max", "max", m=mm, W=32, tau=ta)
