...
BASE = {}
ACC = {"peak": 0, "peakn": 0, "S": 0.0, "L1": 0.0, "nl": 0, "ratio": 0.0, "rcell": 0}
        ...
        if CFG["mode"] == "pca" and m > 0 and qn == T:
            ...
            if two == "none":
                ...
                if two == "exactmax":
                    ...
                    lv = float(2 ** (bits - 1) - 1)
                    if two.endswith("p"):
                        fl = k[0].abs().reshape(h, -1).float()
                        scl = torch.quantile(fl, 0.999, dim=1).view(h, 1, 1)
                        ACC["ratio"] += float(fl.amax(1).mean() / scl.view(h).clamp_min(1e-8))
                        ACC["rcell"] += 1
                    elif two.endswith("h"):
...
def run(dp, kagg, bagg, m=16, W=32, two="none"):
    ...
    with log.open("a", encoding="utf-8") as f:
        f.write(f"{m:4d}  {CFG['two']:8s}  {l4:9.5f}  {l4 - D:+.5f}  {pk:5.1f}  "
                f"{Sm:.4f}  {L1:.4f}\n")
    if ACC["rcell"]:
        print(f"   ratio max/p99.9 = {ACC['ratio'] / ACC['rcell']:.1f}", flush=True)
...
for mm in (8, 16, 24):
    ...
    for tw in ("none", "exactmax", "q8p", "q4p"):
