# -*- coding: utf-8 -*-
"""TEST DECISIF SUR GPT-2 (banc valide : induction +2,95 nat, tete pure couche 11).
L'index trouve-t-il le bloc SOURCE DISTANT, et precisement le bloc du PIC d'attention ?
Configs : dense | oracle (masse vraie) | index PCA deployable. Fracs 0,375 et 0,125."""
...
def make_fwd(li):
    def fwd(self, hidden_states, layer_past=None, attention_mask=None, head_mask=None,
        ...
        if CFG["mode"] in ("oracle", "pca") and m > 0 and qn == T:
            ...
            nb = qn // LB
            if CFG["mode"] == "oracle":
                B = A3[:, :, :nb * LB].view(h, qn, nb, LB).sum(-1)
            else:
                U = BASE[li]
            ...
            ACC["n"] += mask[:, QPOS, 0:SPAN].any(-1).numel()
            pk = A3.argmax(-1)
            pkb = pk // LB
...
def run(mode, frac):
    ...
    print(f"{tag}: perte_copie4={l4:.5f} (Delta {l4-D:+.5f}) "
          f"bloc_source_sel={selp:.1f}% bloc_du_PIC={peakp:.1f}%", flush=True)
    return l4
