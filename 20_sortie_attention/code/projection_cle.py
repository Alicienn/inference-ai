# -*- coding: utf-8 -*-
"""Compression par REDUCTION DE DIMENSION (et non par sous-ensemble de cles).
On stocke les cles projetees par une matrice aleatoire P (d' x D) : d'/D des octets de cles,
calculees une fois au prefill. Score = max sur le bloc de (Pq).(Pk)/sqrt(d').
Comparer a maxip (toutes cles, D dims) et au hasard. Harnais causal corrige."""
...
CFG = {"mode": "dense", "sel": "maxip", "dp": 16, "seed": 0}
...
NB = T // Lb
log = OUT / "projection_cle.txt"
log.write_text(f"{MID} | T={T} W={W} Lb={Lb} m={M} | projection aleatoire d' dims\n",
               encoding="utf-8")
...
def proj(dp):
    if dp not in PROJ:
        PROJ[dp] = torch.randn(dp, D, generator=GEN) / math.sqrt(dp)
    return PROJ[dp]
            ...
            if CFG["mode"] != "m0":
                ...
                if sel == "rand":
                    ...
                    if sel == "proj":
                        dp = CFG["dp"]
                        P = proj(dp)
                        qp = torch.einsum("hqd,pd->hqp", q[0], P)
                        kp = torch.einsum("hkd,pd->hkp", ke[0], P)
...
for dp in (4, 8, 16, 32):
    CFG.update({"sel": "proj", "dp": dp}); res[f"proj_d{dp}"] = float(loss().mean())
CFG.update({"sel": "rand"})
rands = []
for sd in (1000, 2000):
    CFG["seed"] = sd
...
with log.open("a", encoding="utf-8") as f:
    ...
    f.write(f"\nproj_d16 vs maxip : {res['proj_d16'] - res['maxip_D64']:+.4f} nat (25 % des octets de cles)\n")
    f.write(f"proj bat le hasard ? d4={res['proj_d4'] < min(rands)} d8={res['proj_d8'] < min(rands)} "
            f"d16={res['proj_d16'] < min(rands)} d32={res['proj_d32'] < min(rands)}\n")
print("ok")
