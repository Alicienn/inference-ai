...
def run(dp, kagg, bagg, m=16, W=32, two="none", swapk=0):
    global D
    if dp is None:
        CFG["mode"] = "dense"; CFG["W"] = T // 8; CFG["m"] = 0
    else:
        CFG["mode"] = "pca"; CFG["dp"] = dp; CFG["kagg"] = kagg; CFG["bagg"] = bagg
        CFG["W"] = W; CFG["m"] = m; CFG["two"] = two; CFG["swapk"] = swapk; CFG["lab"] = two + (f"-k{swapk}" if swapk else "")
    ...
    with log.open("a", encoding="utf-8") as f:
        f.write(f"{m:4d}  {CFG['lab']:8s}  {l4:9.5f}  {l4 - D:+.5f}  {pk:5.1f}  "
                f"{Sm:.4f}  {L1:.4f}\n")
    print(f"m={m} two={CFG['lab']}: perte={l4:.5f} Delta={l4-D:+.5f} pic={pk:.1f}% "
          f"S={Sm:.4f} L1={L1:.4f}", flush=True)
    rows.append((f"m{m}-{CFG['lab']}", l4 - D, pk, Sm, L1))
    return l4
...
for mm in (8, 16, 24):
    ...
    for tw in ("none", "exactmax", "exactmass", "rand"):
...
print("FIN")
print("### sensibilite : k remplacements (m=16)", flush=True)
for kk in (0, 1, 2, 4):
