# -*- coding: utf-8 -*-
"""LOI D'ECHELLE AVEC LE REGLAGE SATURE (Lb=4) : la fraction de cles lue reste
CONSTANTE a 25 % pendant que T quadruple, la config etant mise a l'echelle
(W=T/8, m=T/32 blocs de 4). Controle : fenetre pure de meme taille (W=T/4)."""
...
DP, NFIT, LB = 8, 256, 4
# (T, W, m) : m>0 -> selecteur ; m=0 -> fenetre pure de meme taille
CONFIGS = [(512, 64, 16), (512, 128, 0), (1024, 128, 32), (1024, 256, 0),
           (2048, 256, 64), (2048, 512, 0)]
OFFSETS = [0, 200000]
CFG = {"mode": "sparse", "sel": "pca", "W": 64, "Lb": LB, "m": 16}
...
log = OUT / "echelle_saturee.txt"
log.write_text(f"{MID} | Lb={LB} d'={DP} | fraction lue = 25 % constante\n"
               "T     W    m   cles  offset  dense    maxip    pca      vs_dense  index\n",
               encoding="utf-8")
...
for off in OFFSETS:
    dens = {}
    for (T, W, M) in CONFIGS:
        if T not in dens:
            CFG.update({"mode": "dense"})
            dens[T] = loss(ALL[off:off + T][None])
        ld = dens[T]
        ids = ALL[off:off + T][None]
        CFG.update({"mode": "sparse", "W": W, "m": M})
        if M == 0:
            lp = lm = loss(ids)
        else:
            CFG.update({"sel": "maxip"}); lm = loss(ids)
            CFG.update({"sel": "pca"}); lp = loss(ids)
        acc.setdefault((T, W, M), []).append((ld, lm, lp))
        with log.open("a", encoding="utf-8") as f:
            f.write(f"{T:5d} {W:4d} {M:4d} {W+M*LB:5d} {off:7d} {ld:8.4f} {lm:8.4f} "
                    f"{lp:8.4f} {lm-ld:+9.4f} {lp-lm:+8.4f}\n")
            f.flush()
        print(f"off={off} T={T} W={W} m={M} vs_dense={lm-ld:+.4f} index={lp-lm:+.4f}", flush=True)
with log.open("a", encoding="utf-8") as f:
    f.write("\n=== moyennes 2 tranches ===\nT     W    m   cles  vs_dense  index\n")
    for (T, W, M), v in acc.items():
        vd = sum(x[1] - x[0] for x in v) / len(v)
        ic = sum(x[2] - x[1] for x in v) / len(v)
        f.write(f"{T:5d} {W:4d} {M:4d} {W+M*LB:5d} {vd:+9.4f} {ic:+8.4f}\n")
print("FIN")
