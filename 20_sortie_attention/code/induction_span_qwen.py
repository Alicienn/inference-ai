# -*- coding: utf-8 -*-
"""Le banc est-il degenere a cause du MODELE ou de la TACHE ?
Meme sonde (span naturel 64 tok repete 4x, T=256) sur Qwen2.5-0.5B.
Mesures : gain d'induction (perte copie4 vs copie1), masse dense sur la COPIE 1
(source distante, niveau uniforme 0,25) et sur la COPIE 3 (source locale)."""
...
BASE = {}
ACC = {"m1": 0.0, "m3": 0.0, "s1": 0.0, "sel1": 0, "n": 0, "nl": 0}
...
def make_fwd(li):
    def fwd(self, hidden_states, position_embeddings=None, attention_mask=None, **kw):
        ...
        if CFG["mode"] == "pca" and m > 0 and qn == T:
            ...
            ACC["m1"] += float(A[:, QPOS, 0:SPAN].sum(-1).mean())
            ACC["m3"] += float(A[:, QPOS, 128:192].sum(-1).mean())
            sp = A * mask
            ACC["s1"] += float(sp[:, QPOS, 0:SPAN].sum(-1).mean())
            ACC["sel1"] += int(mask[:, QPOS, 0:SPAN].any(-1).sum())
...
log = OUT / "induction_span_qwen.txt"
...
def run(frac):
    ...
    if frac is not None:
        nl = max(ACC["nl"], 1)
        msg += (f" | masse copie1={ACC['m1']/nl:.4f} copie3={ACC['m3']/nl:.4f} "
                f"(uniforme 0,250) retenue={ACC['s1']/nl:.4f} "
                f"bloc_sel={100*ACC['sel1']/max(ACC['n'],1):.1f}%")
